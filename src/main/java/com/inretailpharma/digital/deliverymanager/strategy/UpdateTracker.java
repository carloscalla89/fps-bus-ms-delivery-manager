...
public class UpdateTracker extends FacadeAbstractUtil implements IActionStrategy {
    ...
    public Mono<OrderCanonical> process(ActionDto actionDto, Long ecommerceId) {
        ...
        return Flux.fromIterable(utilClass.getClassesToSend())
                .flatMap(objectClass ->
                        ((ITrackerAdapter) context.getBean(objectClass)).evaluateTracker(
                ...
                .flatMap(resp ->
                        UtilFunctions.getSuccessResponseFunction.getMapOrderCanonical(
                                ...
                                utilClass.getFirstOrderStatusName(), iOrderFulfillment.getOrderId(),
                                utilClass.getServiceType(), actionDto.getOrderCancelCode()
                        )
                )
